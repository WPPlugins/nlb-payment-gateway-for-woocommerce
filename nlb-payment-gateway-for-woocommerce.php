	...
	class WC_Gateway_tebank extends WC_Payment_Gateway {
		...
		public function process_payment( $order_id ) {
			...
			if ( '1' === $payment_pipe->getAction() || '4' === $payment_pipe->getAction() ) {
				...
				$payment_pipe->setUdf4( $order->billing_first_name );
				$payment_pipe->setUdf5( $order->billing_last_name );
				$track_id = md5( uniqid() );
				add_post_meta( $order_id, 'tracking_id', $track_id );
				$payment_pipe->setTrackId( $track_id );
				if ( $payment_pipe->performPaymentInitialization() !== $payment_pipe->SUCCESS ) {
					...
					add_post_meta( $order_id, 'payID', $payment_pipe->getPaymentID() );
